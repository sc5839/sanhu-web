...
function Market() {
  ...
  useEffect(() => { fetch('/api/market/boards').then(r => r.json()).then(data => { if (data.error) throw new Error(data.error); setLive(data); setSelected(data.rows[0]?.name || selected) }).catch(() => setError('行情源暂时不可用，正在保留上次成功数据。')) }, [])
  const rows = live?.rows || sectorRows; const row = rows.find(r => r.name === selected) || rows[0]
  useEffect(() => { if (!row?.code) return; fetch(`/api/market/kline?code=${row.code}`).then(r => r.json()).then(data => setKline(data.klines || [])).catch(() => setKline([])) }, [row?.code])
